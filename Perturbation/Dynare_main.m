...
rhot = 0.9; % production shock persistence
...
N_states=3;
N_controls=3;
[U,V,Uss,Vss,UE,VE]= simulate_U_V(A,B,C,D,N,idx_c,idx_k,idx_n,idx_z,idx_theta,idx_u,N_states,N_controls,N_nodes_total,qnodes,oo_,sigmaz,sigmat);
...
for j=2:sim_len
    for m=1:N_nodes_total
       E(j,m)=((param.beta*VE(idx_c_v,j,m).^(-param.gamma).*(((1.0- (param.dss + param.phi1 * (VE(idx_u_v,j,m) -1) + param.phi2/2 * (VE(idx_u_v,j,m) -1)^2)) +  ...
           param.alpha * exp(UE(idx_z_u,j,m)).*(U(idx_k_u,j)).^(param.alpha-1) .* VE(idx_u_v,j,m)^(alpha) .*VE(idx_n_v,j,m)^(1-alpha)))));
    end
end
...
% compute errors
EERES=((param.beta * param.gx^(1-param.gamma) .* E(2:end,:)) .*(param.gx * (V(idx_c_v,2:end).^(-param.gamma))' ).^(-1)) - 1;

max_ee_res=log10(max(abs(EERES)));
mean_ee_res=log10(mean(abs(EERES)));
