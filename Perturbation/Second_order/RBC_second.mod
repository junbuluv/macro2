...
model;
    ...
    B*(1-n)^(-mu)  = c^(-gamma)*((1-alpha)*exp(theta)*u *k(-1)^(alpha)*n^(-alpha));
    

    [name='Capital utilization']
    (exp(z)*phi1 + phi2 * (u-1))  = alpha * exp(theta) * k(-1)^(alpha-1) * n^(1-alpha);
    

    [name='Budget constraint']
    c + gx*k = (1- (dss + exp(z)*phi1 * (u - 1) + phi2/2 * (u-1)^2))*k(-1) + exp(theta)*u*k(-1)^(alpha) * n^(1-alpha);
