...
A = mat.A; %(y_t - yss)
B = mat.B; %(y_t ~ exogenous)
C = mat.C; %(k_t-kss)^2 , (k_t-kss)(z_t-z_ss), (k_t - kss)(theta_t - theta_ss), (z_t -zss)(k_t - kss), (z_t - zss)^2 , (z_t -zss)(theta_t - theta_ss)  ,(theta_t - theta_ss)(theta_t - k_t), (theta_t - theta_ss)^2, (theta_t-theta_ss)(z_t - z_ss)
D = mat.D; 
E = mat.E;
F = mat.F;
G = mat.G;
...
J = mat.J;
    ...
    for j=2:N+burn_in
        ...
        for m=1:N_nodes_total
            ...
            VE(:,j,m)=F*U(:,j)+G*q_nodes(m,:)' + 0.5*H*(kron(V(:,j),V(:,j))) + 0.5*I*kron(q_nodes(m,:)',q_nodes(m,:)') + J*(kron(V(:,j),q_nodes(m,:)'));
        end
    end
