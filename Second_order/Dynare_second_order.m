...
for j=2:sim_len
    for m=1:N_nodes_total
       E(j,m)=((param.beta*VE(idx_c_v,j,m).^(-param.gamma).*(((1.0- (param.dss + param.phi1 * (VE(idx_u_v,j,m) -1) + param.phi2/2 * (VE(idx_u_v,j,m) -1)^2)) +  ...
           param.alpha * exp(UE(idx_z_u,j,m)).*(U(idx_k_u,j)).^(param.alpha-1) .* VE(idx_u_v,j,m)^(alpha) .*abs(VE(idx_n_v,j,m))^(1-alpha)))));
    end
end
...
% compute errors
EERES=((param.beta * param.gx^(1-param.gamma) .* Eq(2:end,:)) .*(param.gx * (V(idx_c_v,2:end).^(-param.gamma))' ).^(-1)) - 1;
