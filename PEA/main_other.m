...
%labor (t+1) - parametrization
n_p = 1- (beta_s/gx .* exp(coef(1) + coef(2).* log(kprime) +  coef(3) .* log(theta_p) + coef(4).* log(z_p))).^(-1/mu);
%capital utilization (t+1) - FOC
u_p = ((alpha*theta_p.*kprime.^(alpha-1).*n_p.^(1-alpha) - z_p.*phi1)./phi2) + 1 ;
%consumption (t+1) - FOC
c_p = (B.*(1-n_p).^(-mu) ./ ((1-alpha)*theta_p.*u_p.*kprime.^(alpha).*n_p.^(-alpha))).^(-1/gamma);
%labor and capital utilization (t)
u_quad = repmat(u,1,n_nodes);
n_quad = repmat(n,1,n_nodes);

%calculate expectation part
e = (1-n_p).^(-mu) ./ (theta.* u_quad.* k.^(alpha).* n_quad.^(-alpha)) .* (theta_p.*u_p.*kprime.^(alpha).*n_p.^(-alpha)) .*...
